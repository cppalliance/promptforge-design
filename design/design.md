...
- [design-mcp-server-residue.md](design-mcp-server-residue.md) - `promptforge-mcp-server`: the unbuilt service - one MCP tool per prompt, the Django fire-and-poll endpoints, declared outputs and output roots, extension wiring, run limits, service installation. The server that exists publishes four fixed built-ins and runs the prompt a caller names to `run_prompt`, and is documented in its own `design-mcp-server.md`
