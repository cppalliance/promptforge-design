...
spawn subagents and search the web for agentic techniques for taking separate web fetch researtch results and combining them into a single document. this is essentially map-reduce. I am wondering, can a model be fine-tuned to perform this? maybe in a streaming way? a fast way? find out search broad and deep
...
create a beautiful report in @promptforge-design ( use @tools-public/how-to/how-to-write-reports.md ) keep inline hyperlinks, inverted pyramid
...
how can such a small model handle everything though? what if the inputs have code fences. C++. Javascript. etc
