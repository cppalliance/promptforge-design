# MCP client / agentic harness work

*2026-08-02 10:34 - transcript 068d3001-b330-4843-93e6-0fbe10f9985e*

## Prompts

how does mcp client work? what does an agentic harness need to implement

should I put the mcp client in its own crate

tell me about this sampling business

so the mcp client just appears as a normal tool call to the model?

but if the mcp client needs the Tool struct and the core crate needs the mcp client isn't that a circular dep

lets build the mcp client crate with @tools-public/tools/architect.md and @tools-public/how-to/vibe-how-to.md

why is this other design so big? what are the pros and cons

is this the consequence of the lua having to name each tool for each h2?

it seems to me the thing it buys us is it saves one round of inference. because if we want to get rid of the indirection we could just look at what the mcp services are available and use those to set the tools.add list based on the prompt's needs stated in prose?

No. What if the prompt has a frontmatter which describes the tools it needs in  english, and then we use a classifier at launch time to map the mcp offerings to what the prompt needs?

there needs to be a way for the lua to say "add everything" from the mcp client. and maybe a way to ask for a tool that matches a description?

spawn multiple subagents and search for everything related to these ideas. classifiers. progressive reveal of tool capabilities in mcp servers. and whatever else you can think of
...
"a cross-encoder reranker over embedding top-k is the untested next step if 0.85 isn't good enough." this is why I wanted infinite running. keep fucking going. but first explain what this means. I have no idea what these terms are. explain everything, assume I am very intelligent but dont know much. What is NLI? what is embedding? should we fine-tune (is fine-tune even a meaningful term here)?

Question about fine-tuning. Instead of going "all the way" is it possible to just fine tune enough to get N% improvement? so we can get a boost in performance without the degeneracy?

also run all jobs async please, so we can keep talking

rerun that cancelewd job

Briefly inform the user about the task result and perform any follow-up actions (if needed).

Hey, but wait a minute, we actually are doing this wrong, because we're just asking, "Is this tool a fit for?" The description. But what we re for the need, what we really wanna know is given a set of n tools Which one is the best fit for a need? That's a different question entirely.

Here's my idea. We do We do a ranking, we order, we sort them using a not so great criteria, like it doesn't have to be perfect, and then we do the point wise on the top three or something, something, some combination of all at once and point wise on a reduced set.

Briefly inform the user about the task result and perform any follow-up actions (if needed).

I dont want LLM in the final result. I want something that runs locally.

what if we build a vocabulary list of like 10,000 words and we do some kind of deterministic pass first just doing word comparison and doing a count

Briefly inform the user about the task result and perform any follow-up actions (if needed).

what if we divide the need and the description strings into (noun,verb) lists and compare nouns and verbs separately in embedding space and use that as additional signal

And here's another idea. we can fine tune verbs separately from nouns.

And here's another idea. What if we take all of the eval pairs and we generalize to find out what N categories they have in common (for small N) and then we have a separate categorizer and we fine tune on that and then the algorithm becomes, first we determine category and then we choose a separate fine-tuned model for that specific category. since they are all in the same category the fine tune will have less chance of degeneracy. Basically we partition the tool space so each reranker/classifier can be more narrow
...
so to be clear out system takes (need, description) strings and outputs a number from 0 to 1 ?

should this be a feature of the mcp client? i.e. offer the api to do this calculation

okay tell me our current best algorithm for calculating (need, catalog) ?

why do you tell me bge-small-en-v1.5 and then tell me bge-base is up to 2 points better? why mention the bge-small at all?

how small

I'd like 10x more real-world eval data. do it asynchronously. in fact do everything asynchronously.

9,922 sounds like the right number

can't we do 10,000 test cases

spawn a subagent and look for a source of actual need strings

and do the other stuff in the async please. I want async at all times so we can keep talking. 

if you are going to generate need strings you should do it using multiple models. chatgpt, claude, gemini, cursor, etc
...
I think a good way to make need strings is to take the entire catalog corpus, and for each tool description, restate the description in various ways. you could get 5 need strings out of one description.

think about it - when the author goes to write the lua need strings, they will likely just copy or paraphrase the description from the tool they already have which they want to use

We should optimize for identifying when there are duplicates in the catalog, and fail loud there. Having two of the same tool in my opinion is a configuration error, except when the user intentionally imports foreign mcp servers. and even in that case there has to be a way to disambiguate. For example we could *require* in that case that the user has to use the LLM to do the tool select, not the lua.

Briefly inform the user about the task result and perform any follow-up actions (if needed).

What do you think of this: the promptgate executor can have a built-in tool choose_mcp_tool and this takes the need string and launches a fresh subcontext to make the decision, and it can also return the rationale

ha.. no you dont quite get it. it is not "an escalation path" it is the ONLY path, the author opts-in:
...
My guess, and I am not certain, is that choose_mcp_tool will return a tool description. Perhaps more than one (maybe thise could be a separate choose_mcp_tools). And it could return an error "no tools available." The point is that this is complementary to the static approach, not a replacement.
...
don't we need to generate the new need strings for the corpus?

well choose_mcp_tools is already paying for inference in the fresh subcontext so we might as well return a shortlist and then have the subcontext select the best fit

actually we might not need a subcontext for the choosing. The main context performs the toolcall, we get a shortlist and then we just inject the shortlist into the main context rewrite. Then the main context has all ~3 tools to pick from. And it has the full history.

the nice thing about the main context is that the author already has all the mechanisms to control it. they can select the model. they can opt-out of the context rewrite. if we did the subcontext then we'd have to answer the question "which model".

It would be nice to have something where, a step uses a reasoning model to select the tool and then it does the context-clearing goto (a promptforge feature) and passes the tool descriptor into the next context, and that context can use a lesser model

yes

Let me explain to you what I want. I want like, I want a fresh. Directory. As a sibling, and I want all the training, I want all the evaluation data, like I want a beautiful clean dataset, and that's like, that's the final dataset that we can use. It's got the distractors, it's got the proper need strings. You said, I think you said two for each descriptor, or did you say three? And I wanna have it nice and clean. So that we can prove The numbers. And then there's enough there we're gonna do the implementation in the other repo. And I want a design doc. We need a, a design doc. The findings is nice, but we need the final design doc that explains it has enough to explain. I'm gonna give you the architect, and you can see what the architect wants. @tools-public/tools/architect.md

design-mcp-toolpicker

Plan and I want the design first, then build the dataset, then update thte design doc to have the reproduced numbers. How do you feel about that?

I think you are confused. The design doc is the design of the promptforge-mcp-toolpicker crate. That means, the tools.add, the tools.add_need, the choose_mcp_tool

I also want the plan to build out the final spike in the sibling dir (so we can run it later if things change), and to have the nice dataset with the need strings. it can copy over the dataset and put it in a very efficient representation (I dont want 16,000 files)

should we embed the mcp client?

the argument for making it a separate crate is that a coding LLM only has to consider a smaller amount of code, and it is morely to completely fit in a single context window.

this should go in @promptforge-design//study-mcp-toolpicker

what do you mean? that IS a separate sibling dir...where were you going to put it otherwise?

the plan needs to mine this entire chat history to understand and explain the rationale this should go in a separate rationale.md document. it explains the tool scoping, how the model can pick the tool using choose_mcp_tool, the goto, all ofthat

run

run @tools-public/tools/chatlight.md write to @chat-logs/vinnie-falco
...
StrReplace-edited design docs (path only): `c:\Users\Vinnie\src\cursor\promptforge-design\mcp-classifier-spike\report\FINDINGS.md`
