...
- [design-macros.md](design-macros.md) - `promptforge-macros`: the `#[tool]` attribute, its expansion, schema and Lua binding derivation
...
- [design-mcp.md](design-mcp.md) - `promptforge-mcp`: the MCP prompt surface, progress notifications, the Django endpoints, `prompts.toml`
...
flowchart TB
    ...
    subgraph service [promptforge-mcp: the only MCP server]
...
14. Prompt frontmatter - inert YAML at the head of a prompt file, parseable without running code: name, description, keywords, parameter schema, canonical tool names required, output declarations, progress templates, version
...
### One Cargo workspace, six crate roles
Workspace named `promptforge`; crate names carry no `-rs` suffix. Crates: `promptforge` (library), `promptforge-gateway`, `promptforge-mcp`, `promptforge-cli`, `promptforge-macros` for the tool attribute macro, and one `promptforge-ext-NAME` per extension. The bare `promptforge` name goes to the library rather than to a binary or a facade, following the `tokio`, `serde`, `axum`, and `clap` convention where the workspace name and the core library coincide and everything else takes a suffix. Tension: more crates to version and release together.
...
### promptforge-mcp is the only MCP server, and it exposes prompts
A connecting client sees prompts on our MCP surface and never tools. Prompt execution is the product; without this server a caller could only use the command line. Cursor already has web search and must not be offered a second one. Tension: a client wanting a bare tool has to go through a prompt.

### Each enabled prompt is its own MCP tool, on the tools primitive
One tool per prompt rather than a single dispatcher taking a prompt name. A calling model selects from names, descriptions, and typed input schemas, and a dispatcher hides all three behind a string the model has to guess. Generation is free because frontmatter already carries the name, the description, the keywords, and a JSON Schema for parameters. Prompts ride MCP's tools primitive rather than its prompts primitive, because `prompts/get` returns text for the client to execute and would hand our control flow to Cursor, which is the opposite of what this system does. Tension: forty prompts is forty tool entries in every connected client's context budget, and the ceiling on that is unmeasured.
...
### A tool attribute macro derives the schema
The macro derives each tool's JSON schema from its argument struct via `schemars` and emits its registry entry, the same mechanism `rmcp` uses. Tension: macro failures surface as compile errors that read worse than plain code.
...
- No prompt-invented tool vocabulary - a prompt names tools from the canonical set and configuration binds each name to an extension
...
## Build Path
Ordered steps: 1. the gateway, because it improves the existing Python stack with only a configuration change; 2. the library and the macro, the `Extension` trait among the first things written because every later step depends on its shape, ending in an integration test that links the library against the fake gateway and the recording extension, which is the fastest end-to-end exercise and the reason no `--local` flag is needed; 3. the MCP server, the first thing that can execute a prompt for a caller; 4. the CLI as a client of it; 5. extension crates, one at a time, `promptforge-ext-search` first because it is the smallest complete exercise of the trait, then `promptforge-ext-paperstore`, whose Postgres backend is in scope and does not exist yet in either repository. Classifier support arrives as `promptforge-ext-classify` after step 2, gated on an offline FP32 ONNX export reaching numerics parity with PyTorch to three decimals before any prompt calls it. On a single-card host, services start in order: vLLM with explicit headroom, then the in-process classifiers, then any speech stack.
...
| Tool registration and macro | medium | Mechanism is proven in `rmcp`, but no extension crate is built yet |
...
- Crate versions confirmed against crates.io on 2026-07-25: `rmcp` 2.2.0 stable with 3.0.0-beta.2 published 2026-07-24; `syn` 2.0.119 pinned against `syn` 3.0.2 because `darling` 0.23.0 requires `syn` ^2.0.15; `schemars` 1.2.1; `trybuild` 1.0.118; `sqlx` 0.9.0; `reqwest` 0.13.4; `ort` 2.0.0-rc.12; `fastembed` 5.17.2; `tokenizers` 0.23.1
